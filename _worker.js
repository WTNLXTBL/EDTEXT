...
const RelayIPs = ['cdn.xn--b6gac.eu.org', 'cdn-all.xn--b6gac.eu.org', 'workers.cloudflare.cyou'];

// if you want to use ipv6 or single RelayIP, please add comment at this line and remove comment at the next line
let RelayIP = RelayIPs[Math.floor(Math.random() * RelayIPs.length)];
// use single RelayIP instead of random
// let RelayIP = 'cdn.xn--b6gac.eu.org';
// ipv6 RelayIP example remove comment to use
// let RelayIP = "[2a01:4f8:c2c:123f:64:5:6810:c55a]"
...
export default {
	 ...
	 * @param {{UUID: string, RelayIP: string, DNS_RESOLVER_URL: string, NODE_ID: int, API_HOST: string, API_TOKEN: string}} env
			...
			if (!upgradeHeader || upgradeHeader !== 'websocket') {
				...
				switch (url.pathname) {
					...
					default:
						...
						newHeaders.set('referer', 'https://www.google.com/search?q=EDTEXT');
...
async function handleTCPOutBound(remoteSocket, addressRemote, portRemote, rawClientData, webSocket, vlessResponseHeader, log,) {
	...
	async function retry() {
		const tcpSocket = await connectAndWrite(RelayIP || addressRemote, portRemote)
...
function getVLESSConfig(UserIDs, hostName) {
	...
	const output = UserIDArray.map((UserID) => {
		...
		const vlessSec = 'vless://' + UserID + '@' + RelayIP + commonUrlPart;
...
<a href='https://github.com/3Kmfi6HP/EDTEXT' target='_blank'>EDTEXT - https://github.com/3Kmfi6HP/EDTEXT</a>
...
<a href='v2rayng://install-config?url=${encodeURIComponent(subbestip)}' target='_blank'>v2rayNG优选IP自动</a></p>`;
	...
	const htmlHead = `
  <head>
	<title>EDTEXT: VLESS configuration</title>
	<meta name='description' content='This is a tool for generating VLESS protocol configurations. Give us a star on GitHub https://github.com/3Kmfi6HP/EDTEXT if you found it useful!'>
	<meta name='keywords' content='EDTEXT, cloudflare pages, cloudflare worker, severless'>
	<meta name='viewport' content='width=device-width, initial-scale=1'>
	<meta property='og:site_name' content='EDTEXT: VLESS configuration' />
	<meta property='og:type' content='website' />
	<meta property='og:title' content='EDTEXT - VLESS configuration and subscribe output' />
	...
	<meta name='twitter:title' content='EDTEXT - VLESS configuration and subscribe output' />
	...
	const output = UserIDArray.flatMap((UserID) => {
		const httpConfigurations = Array.from(portSet_http).flatMap((port) => {
			if (!hostName.includes('pages.dev')) {
				...
				return RelayIPs.flatMap((RelayIP) => {
					const vlessSecHttp = 'vless://' + UserID + '@' + RelayIP + ':' + port + commonUrlPart_http + urlPart + '-' + RelayIP + '-EDTEXT';
		...
		const httpsConfigurations = Array.from(portSet_https).flatMap((port) => {
			...
			return RelayIPs.flatMap((RelayIP) => {
				const vlessSecHttps = 'vless://' + UserID + '@' + RelayIP + ':' + port + commonUrlPart_https + urlPart + '-' + RelayIP + '-EDTEXT';
